...
def credibility(comparables_a, comparables_b, concordance, discordance,
                    with_denominator, only_max_discordance, use_partials):

    def _credibility_index(x, y, with_denominator, only_max_discordance,
                               use_partials):
        if use_partials:
        ...
        if set(discordance_values) == set([0]):  # only zeros
            ...
            if factors == []:
                ...
                c_idx = concordance[x][y] * functools.reduce(lambda f1, f2: f1 * f2,
                                                   factors)
        return c_idx
    ...
    for a in comparables_a:
        for b in comparables_b:
            credibility[a][b] = _credibility_index(a, b, with_denominator,
                                                       only_max_discordance,
                                                       use_partials)
            if two_way_comparison:
                credibility[b][a] = _credibility_index(b, a, with_denominator,
                                                           only_max_discordance,
                                                           use_partials)
    return credibility
